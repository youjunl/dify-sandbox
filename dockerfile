...
RUN echo "http://mirrors.aliyun.com/debian testing main" > /etc/apt/sources.list \
    && apt-get update \
...
RUN chmod +x /main /env /entrypoint.sh \
    && pip3 install --no-cache-dir httpx==0.27.2 requests==2.32.3 jinja2==3.1.6 PySocks httpx[socks] python-docx dataclasses numpy pandas jsons ast PyPDF2\
